"""
EAA CONTROL V6 - SUPERVISOR (IMMORTAL VERSION)
==============================================
...
MAIN_SERVER = os.path.join(SCRIPT_DIR, "eaa_control_v6.py")
...
def send_crash_notification(reason="Server Crashed"):
    ...
    subject = f"EAA Control V6 - {reason}"
    
    body = f"""
{'='*60}
   EAA CONTROL V6 - SUPERVISOR NOTIFICATION
{'='*60}
...
1. Server crashed or was stopped
2. Supervisor detected it
3. Server was automatically restarted
4. New tunnel was created
5. You'll receive new credentials shortly
...
def ignore_signal(sig, frame):
    ...
    print("\n[SUPERVISOR] Ctrl+C ignored! Close this window to stop.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(MAIN_SERVER):
        print(f"\n[ERROR] Main server not found: {MAIN_SERVER}")
        print("[ERROR] Make sure eaa_control_v6.py is in the same folder!")
        sys.exit(1)
    ...
    print("\n[SUPERVISOR] Monitoring... (Ctrl+C is disabled)")
    print("-" * 60)
    ...
    while True:
        try:
            ...
            print(f"[SUPERVISOR] Error (ignored): {e}")
            time.sleep(5)

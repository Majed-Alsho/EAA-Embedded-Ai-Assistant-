"""
EAA CONTROL SYSTEM V6 - COMPLETE
=================================
- All V5 features (screenshot, click, type, files, shell)
- NEW: Audio control, Webcam, OCR, Recording, Media
- NEW: File search, Network tools, Image operations
- Email notifications when tunnel starts/restarts
...
def set_volume(level):
    try:
        ...
        try:
            ...
            return True, "Volume adjusted (limited control)"
        except Exception as e:
            return False, str(e)
...
class ControlHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/version":
            self.send_json({"version": "6.0.0", "features": {"screenshot": PIL_AVAILABLE, "remote_control": PYAUTOGUI_AVAILABLE, "webcam": CV2_AVAILABLE, "ocr": PYTESSERACT_AVAILABLE, "tts": TTS_AVAILABLE, "gpu": GPU_AVAILABLE}})
            return
        ...
        if path == "/eaa/status":
            self.send_json({"suc": True, "running": eaa_process is not None and eaa_process.poll() is None, "tunnel": tunnel_url})
            return
    ...
    def do_POST(self):
        ...
        if path == "/mouse/drag":
            if not PYAUTOGUI_AVAILABLE:
                self.send_json({"suc": False, "err": "pyautogui not available"})
                return
            pyautogui.moveTo(data.get("startX", 0), data.get("startY", 0))
            pyautogui.drag(data.get("endX", 0) - data.get("startX", 0), data.get("endY", 0) - data.get("startY", 0), duration=data.get("duration", 0.5), button=data.get("button", "left"))
            self.send_json({"suc": True})
            return
        ...
        if path == "/file/list" or path == "/list":
            ...
            try:
                ...
                for item in os.listdir(dir_path):
                    ...
                    items.append({"name": item, "is_dir": os.path.isdir(full), "size": os.path.getsize(full) if os.path.isfile(full) else 0, "modified": datetime.fromtimestamp(os.path.getmtime(full)).isoformat() if os.path.exists(full) else None})
        ...
        if path == "/app/launch":
            ...
            apps = {"chrome": "start chrome", "firefox": "start firefox", "edge": "start msedge", "vscode": "code", "notepad": "notepad", "explorer": "explorer", "spotify": "start spotify", "discord": "start discord", "calculator": "calc", "settings": "start ms-settings:", "task_manager": "taskmgr"}
            if path_arg:
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  EAA CONTROL SYSTEM V6 - COMPLETE")
